...
train_df = pd.read_excel("Output/preprocessed_train.xlsx").fillna("")
val_df = pd.read_excel("Output/preprocessed_val.xlsx").fillna("")
...
# md.train_evaluate_model(lstm_model, X_train_tf_idf, y_train, X_val_tf_idf, y_val, 'Output/Saved Models/LSTM.h5')
# print("\n")

# #2 Bidirectional LSTM
# print("#2 Bidirectional LSTM model \n")
# bidirectional_lstm_model = md.build_lstm_bidirectional(input_shape=(1, model_shape[1]))
# md.train_evaluate_model(bidirectional_lstm_model, X_train_tf_idf, y_train, X_val_tf_idf, y_val, 'Output/Saved Models/Bidirectional LSTM.h5')
# print("\n")

# #3 Simple RNN
# print("#3 Simple RNN model \n")
# simple_rnn_model = md.build_simple_rnn(input_shape=(1, model_shape[1]))
# md.train_evaluate_model(simple_rnn_model, X_train_tf_idf, y_train, X_val_tf_idf, y_val, 'Output/Saved Models/Simple RNN.h5')
# print("\n")

# #4 GRU
# print("#4 GRU model \n")
# gru_model = md.build_gru(input_shape=(1, model_shape[1]))
# md.train_evaluate_model(gru_model, X_train_tf_idf, y_train, X_val_tf_idf, y_val, 'Output/Saved Models/GRU.h5')
# print("\n")
...
# md.train_evaluate_model(embedding_lstm_model, X_train_embedding, y_train, X_val_embedding, y_val, 'Output/Saved Models/Embedding LSTM.h5', batch_size=256, use_early_stopping= False)

#6 Transformer
print("#6 Transformer \n")
transformer_model = md.build_transformer(vocab_size=vocab_size, embedding_dim=20, num_heads=5, num_transformer_layers=2, input_length= X_train_embedding.shape[1])
md.train_evaluate_model(transformer_model, X_train_embedding, y_train, X_val_embedding, y_val, 'Output/Saved Models/Transformer.h5', batch_size=256, use_early_stopping= False)
